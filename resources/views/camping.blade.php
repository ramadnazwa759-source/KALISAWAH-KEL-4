...
@section('content')
    @php
        // HERO CONFIGURATION
        $heroImage = (isset($hero->image) && $hero->image) ? asset('storage/' . $hero->image) : asset('images/camping.jpg');
        $heroTitle = $hero->title ?? 'Camping Seru';
        $heroSubtitle = $hero->subtitle ?? 'Bernapas Bersama Alam';
        $heroDescription = $hero->description ?? 'Nikmati malam bertabur bintang di area camping eksklusif Kalisawah Adventure. Dikelilingi udara segar, pepohonan rindang, dan gemericik sungai yang menenangkan.';
        $heroButtonText = $hero->button_text ?? 'Lihat Paket';
        $heroButtonLink = $hero->button_link ?? '#paket-camping';
    @endphp
