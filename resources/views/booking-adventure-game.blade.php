...
@section('title', 'Booking Adventure Game - Kalisawah Adventure')

@section('content')
    @php
        // Get selected paket from query param
        $selectedPaketName = request('paket', 'SHOOTING TARGET');
        $selectedPaket = $pakets->where('nama_paket', $selectedPaketName)->first() ?? $pakets->first();
        
        // HERO CONFIGURATION (Dynamic from $selectedPaket if available)
        $heroImage = asset('images/outbond.jpg');
        $heroTitle = 'ADVENTURE GAME';
        $heroSubtitle = $selectedPaket ? $selectedPaket->nama_paket : 'Siapkan Ketangkasanmu Bersama Kalisawah Adventure!';
        
        if ($selectedPaket && str_contains(strtolower($selectedPaket->nama_paket), 'panahan')) {
            $heroSubtitle = '“Rasakan sensasi memanah dengan pengalaman yang aman, seru, dan menyenangkan.”';
        } elseif ($selectedPaket && str_contains(strtolower($selectedPaket->nama_paket), 'shooting')) {
            $heroSubtitle = '“Uji akurasi dan fokusmu dengan paket shooting target yang menantang.”';
        }
    @endphp
    ...
    <section class="relative h-[80vh] min-h-[600px] w-full overflow-hidden flex items-center justify-center text-center">
        ...
        <div class="relative z-10 px-6 max-w-5xl mx-auto pt-20">
            <h1 class="text-white text-4xl md:text-6xl lg:text-7xl font-black mb-6 leading-tight uppercase tracking-tight drop-shadow-2xl">
                {!! $heroTitle !!}
                @if($selectedPaket)
                    <br><span class="text-secondary">{{ $selectedPaket->nama_paket }}</span>
                @endif
            </h1>
    ...
    <section class="py-24 px-6 bg-[#F8FAFC]">
        <div class="max-w-[850px] mx-auto">
            ...
            <form id="bookingForm" method="GET" onsubmit="return false;" class="space-y-10">
                @csrf
                <input type="hidden" id="paket_hidden" value="{{ $selectedPaket ? $selectedPaket->nama_paket : 'Adventure Game' }}">
                    ...
                    <div class="space-y-6 pt-6">
                        ...
                        <div id="selected_packages_container" class="space-y-4">
                            ...
                            <div class="bg-white p-6 md:p-8 rounded-[2rem] border border-gray-200 flex items-center justify-between gap-4 shadow-sm group hover:shadow-md transition-all duration-300">
                                <div class="flex-1">
                                    <span class="block font-bold text-dark-navy text-lg uppercase tracking-wide group-hover:text-primary transition-colors">{{ $selectedPaket->nama_paket }}</span>
                                    <span class="block text-xs text-gray-400 font-bold mt-1">Rp {{ number_format($selectedPaket->harga, 0, ',', '.') }} / Paket</span>
                                </div>
                                <div class="flex items-center gap-3 bg-gray-50/50 p-2 rounded-2xl border border-gray-100">
                                    ...
                                    <span id="qty_display" class="w-10 text-center font-black text-primary text-lg">1</span>
                                    <button type="button" onclick="updatePackageQty(1)" 
                                        class="w-10 h-10 rounded-xl bg-white border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-secondary hover:text-white transition-all active:scale-90 font-bold text-xl shadow-sm">+</button>
                                </div>
                            </div>
    ...
    <script>
        ...
        const selectedPackage = {
            id: {{ $selectedPaket ? $selectedPaket->id : 'null' }},
            name: "{{ $selectedPaket ? $selectedPaket->nama_paket : 'Adventure Game' }}",
            price: {{ $selectedPaket ? $selectedPaket->harga : 0 }}
        };
        ...
        bookingForm.addEventListener('submit', async function(e) {
            ...
            if (!selectedPackage.id) {
                alert('Paket tidak valid. Silakan kembali dan pilih paket yang benar.');
                return;
            }
            ...
            const payload = {
                ...
                jam: data.jam,
                jumlah_pengunjung: currentQty, // Untuk game, jumlah pengunjung = qty paket
                catatan: '', // Tidak ada field catatan
                paket: [
                    {
                        paket_wisata_id: selectedPackage.id,
                        qty: currentQty
                    }
                ],
                fasilitas: []
